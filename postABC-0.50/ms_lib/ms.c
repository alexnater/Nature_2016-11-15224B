...
int mssim(int argc, char **argv, int npars, double **pardist, int *nsegs, double **posarray, char ***haplotypes)
{
	int i, k, howmany, segsites, locus=0 ;
	char **cmatrix(), **tbsparamstrs ;
	FILE *pf, *fopen() ;
	...
	if( ntbs > 0 )  for( k=0; k<ntbs; k++) sprintf(tbsparamstrs[k], "%.12f", pardist[k][locus]); // scanf(" %s", tbsparamstrs[k] );
...
//	     list = cmatrix(pars.cp.nsam, pars.mp.segsitesin+1 ) ;
    ...
    while( howmany-count++ ) {
	   if( (ntbs > 0) && (count >1 ) ){
	         for( k=0; k<ntbs; k++)	sprintf(tbsparamstrs[k], "%.12f", pardist[k][locus]); 
//	         for( i=0; i<argc; i++) printf("%s ", argv[i]);
//	         printf("\nlocus: %d\n", locus);
	...
	char **list;
	if( pars.mp.segsitesin ==  0 ) list=cmatrix(pars.cp.nsam,maxsites+1);
	else list=cmatrix(pars.cp.nsam, pars.mp.segsitesin+1 );
        segsites=gensam( list, &probss, &tmrca, &ttot );
	nsegs[locus]=segsites;
	size_t blocksize=segsites * sizeof(double);
	posarray[locus]=(double *)malloc(blocksize); // allocate memory for array of positions.
	memcpy(posarray[locus], posit, blocksize); // copy array of positions to new location.
	haplotypes[locus]=list;
	locus++;
